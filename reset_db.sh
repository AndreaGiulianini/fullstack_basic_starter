...
set -eu
PASS=$(docker exec mysql bash -c 'echo $MYSQL_ROOT_PASSWORD')
DB=$(docker exec mysql bash -c 'echo $MYSQL_DATABASE')
docker exec mysql bash -c "mysql -h localhost -u root -p$PASS -D $DB -s -N -e \"SET SESSION group_concat_max_len = 8192; SET @t = COALESCE( ( SELECT CONCAT('drop table ', GROUP_CONCAT(a.table_name)) FROM information_schema.tables a WHERE a.table_schema = 'lima' AND a.table_type = 'BASE TABLE' ), 'SELECT 1;' ); PREPARE s FROM @t; SET FOREIGN_KEY_CHECKS = 0; EXECUTE s; SET FOREIGN_KEY_CHECKS = 1;\""
